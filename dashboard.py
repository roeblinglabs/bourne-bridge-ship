...
from vessel_analysis import (analyze_vessel, BOURNE_BRIDGE_PIERS, BRIDGE_LAT, BRIDGE_LON,
                             predict_trajectory, assess_collision_risk, calculate_allision_probability)

# Page configuration
st.set_page_config(
    page_title="Bourne Bridge Vessel Monitor",
    page_icon="🚢",
    layout="wide"
)
...
with title_col:
    st.title("🌉 Bourne Bridge Vessel Collision Monitoring System")
...
def get_mock_ships_fallback():
    ...
    mock_ships = [
        {'name': 'DEMO VESSEL 1', 'mmsi': '000000001', 'type': 'Cargo',
         'Latitude': 41.72, 'Longitude': -70.62,
         'Sog': 8.0, 'Cog': 135.0, 'ShipType': 'Cargo',
         'Dimension': {'A': 75, 'B': 75, 'C': 12, 'D': 12}},
        {'name': 'DEMO VESSEL 2', 'mmsi': '000000002', 'type': 'Passenger',
         'Latitude': 41.65, 'Longitude': -70.70,
         'Sog': 12.0, 'Cog': 90.0, 'ShipType': 'Passenger',
...
with col1:
    ...
    # Create map centered on Bourne Bridge
    m = folium.Map(
...
with col2:
    ...
    if ships:
        ...
        for ship in sorted_ships:
            ...
            with st.expander(f"{risk_emoji} {ship['name']} - {risk_level}", expanded=False):
                ...
                # Simplify pier name display
                pier_id = analysis['closest_pier_id']
                if 'North' in pier_id or '1' in pier_id:
                    pier_display = "Pier 1"
                elif 'South' in pier_id or '2' in pier_id:
                    pier_display = "Pier 2"
                else:
    ...
    with st.expander("View Details", expanded=False):
        ...
        st.markdown("""
        • **Name:** Bourne Bridge
        • **Location:** Cape Cod Canal, Massachusetts (41.7447°N, 70.5993°W)
        • **Year Built:** 1935
        • **Year Reconstructed:** [NBI - if applicable]
        ...
        st.markdown("""
        • **Main Span Length:** 616 feet
        • **Total Bridge Length:** [NBI] feet
        • **Deck Width:** [NBI] feet
        • **Roadway Width:** [NBI] feet
        • **Vertical Clearance Over:** 135 feet
        • **Vertical Clearance Under:** N/A (bridge over water)
        • **Horizontal Clearance:** [NBI] feet
        • **Number of Spans:** [NBI]
        """)
        
        st.markdown("### Traffic & Importance")
        st.markdown("""
        • **Average Daily Traffic:** [NBI] vehicles
        • **Detour Length:** [NBI] miles (if bridge closed)
        • **Functional Classification:** [NBI]
        """)
    ...
    with st.expander("View Details", expanded=False):
        ...
        st.markdown("""
        • **Chart Reference:** NOAA Chart 13236 (Cape Cod Canal)
        • **Channel Width:** 540 feet [NOAA Chart 13236]
        • **Maintained Depth:** 32 feet MLW (Mean Low Water) [NOAA]
        • **Project Depth:** 32 feet [NOAA]
        • **Actual Depth Range:** 32-35 feet (varies by location) [NOAA]
        """)
        
        st.markdown("### Clearances")
        st.markdown("""
        • **Vertical Clearance:** 135 feet MHW (Mean High Water) [NOAA]
        • **Horizontal Clearance:** 616 feet (main span)
        """)
        
        st.markdown("### Currents & Tides")
        st.markdown("""
        • **Maximum Current:** [NOAA - Tidal Current Tables] knots (ebb/flood)
        • **Tidal Range:** [NOAA] feet (Mean Range)
        • **Current Pattern:** Bidirectional (tidal flow through canal)
        """)
        
        st.markdown("### Vessel Traffic")
        st.markdown("""
        • **Annual Transits:** [ESTIMATE or USACE data] vessels
        • **Typical Vessels:** Tugs, barges, fishing vessels, ferries, recreational
        • **Speed Limit:** 10 knots (Cape Cod Canal regulation)
        ...
        st.markdown("""
        • **Number of Piers:** 2
        • **Pier 1 Location:** [ESTIMATE from Google Maps] (lat/lon)
        • **Pier 2 Location:** [ESTIMATE from Google Maps] (lat/lon)
        • **Water Depth at Piers:** ~35 feet [ESTIMATE from charts]
        ...
        st.markdown("""
        *📊 Data sources:*
        - *Navigation: NOAA Nautical Chart 13236*
        - *Pier capacity: Placeholder - requires structural engineering analysis*
        - *Pier coordinates: Approximate (measured from satellite imagery)*
